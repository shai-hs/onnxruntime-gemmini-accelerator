...
if (onnxruntime_MINIMAL_BUILD AND NOT onnxruntime_EXTENDED_MINIMAL_BUILD)
  message(FATAL_ERROR "GEMMINI EP can not be used in a basic minimal build. Please build with '--minimal_build extended'")
endif()
...
#                    "Run `sudo apt install uuid-dev` if you need to test ML Program related GEMMINI EP code. ")
#  endif()
#endif()
#
...
if(LINUX)
  find_library(LibGEMMIMI NAMES gemmini)
  find_path(LibGEMMINI_INCLUDE_DIR NAMES gemmini.h)
  if (NOT LibGEMMINI_INCLUDE_DIR)
    message(FATAL "gemmini.h was not found as is required for ML Program support. "
                    "Run `sudo apt install uuid-dev` if you need to test ML Program related GEMMINI EP code. ")
  endif()
endif()
...
file(GLOB onnxruntime_providers_gemmini_public_headers CONFIGURE_DEPENDS
  "${ONNXRUNTIME_INCLUDE_DIR}/core/providers/gemmini/*.h"
)

file(GLOB
  onnxruntime_providers_gemmini_cc_srcs_top CONFIGURE_DEPENDS
  "${ONNXRUNTIME_ROOT}/core/providers/gemmini/*.h"
  "${ONNXRUNTIME_ROOT}/core/providers/gemmini/*.cc"
)
...
set(onnxruntime_providers_gemmini_cc_srcs
  ${onnxruntime_providers_gemmini_cc_srcs_top}
#  ${onnxruntime_providers_gemmini_cc_srcs_nested}
  ${onnxruntime_providers_shared_utils_cc_srcs}
#  ${onnxruntime_providers_coreml_objcc_srcs}
)

source_group(TREE ${ONNXRUNTIME_ROOT} FILES ${onnxruntime_providers_gemmini_cc_srcs})
source_group(TREE ${ONNXRUNTIME_INCLUDE_DIR} FILES ${onnxruntime_providers_gemmini_public_headers})

onnxruntime_add_static_library(onnxruntime_providers_gemmini
  ${onnxruntime_providers_gemmini_public_headers}
  ${onnxruntime_providers_gemmini_cc_srcs}
  #${coremltools_srcs}
)

onnxruntime_add_include_to_target(onnxruntime_providers_gemmini
...
#endif()
#target_include_directories(onnxruntime_providers_gemmini PRIVATE ${FP16_INCLUDE_DIRS})
...
if (APPLE)
  target_compile_definitions(onnxruntime_providers_gemmini PRIVATE __APPLE__)
endif()
...
target_include_directories(onnxruntime_providers_gemmini PRIVATE
                            ${GEMMINI_ROOT}/driver/include
                            ${GEMMINI_ROOT}/runtime/include
)

add_library(libgemmini-driver STATIC IMPORTED)
add_library(libgemmini-runtime STATIC IMPORTED)
set_target_properties(libgemmini-driver PROPERTIES IMPORTED_LOCATION ${GEMMINI_ROOT}/driver/lib/libgemmini-driver.a)
set_target_properties(libgemmini-runtime PROPERTIES IMPORTED_LOCATION ${GEMMINI_ROOT}/runtime/lib/libgemmini-runtime.a)

target_link_libraries(onnxruntime_providers_gemmini PRIVATE libgemmini-driver libgemmini-runtime)

if (LINUX)
  target_link_libraries(onnxruntime_providers_gemmini PRIVATE uuid)
endif()



if (APPLE)
  target_link_libraries(onnxruntime_providers_gemmini PRIVATE "-framework Foundation" "-framework CoreML")
endif()

add_dependencies(onnxruntime_providers_gemmini ${onnxruntime_EXTERNAL_DEPENDENCIES})

set_target_properties(onnxruntime_providers_gemmini PROPERTIES CXX_STANDARD_REQUIRED ON)
set_target_properties(onnxruntime_providers_gemmini PROPERTIES FOLDER "ONNXRuntime")
target_include_directories(onnxruntime_providers_gemmini PRIVATE ${ONNXRUNTIME_ROOT} ${gemmini_INCLUDE_DIRS})
set_target_properties(onnxruntime_providers_gemmini PROPERTIES LINKER_LANGUAGE CXX)

if (NOT onnxruntime_BUILD_SHARED_LIB)
  install(TARGETS onnxruntime_providers_gemmini
          ARCHIVE   DESTINATION ${CMAKE_INSTALL_LIBDIR}
          LIBRARY   DESTINATION ${CMAKE_INSTALL_LIBDIR}
          RUNTIME   DESTINATION ${CMAKE_INSTALL_BINDIR}
          FRAMEWORK DESTINATION ${CMAKE_INSTALL_BINDIR})
